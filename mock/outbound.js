...
// --- Address Generation Helpers (copied from inbound.js for consistency) ---
...
const streetNames = ['人民路', '解放路', '中山路', '和平街', '建设大道', '科技园路', '创新路', '花园小区', '幸福里', '阳光大街', '世纪大道', '沿江路', '大学城路', '南京路', '淮海中路', '长安街'];
const streetSuffixes = ['号院', '大厦A座', '国际广场', '物流园C区', '工业园', '新村', '写字楼', '商务中心', '生活广场', '创意园区'];
const roadTypes = ['路', '街', '大道', '巷', '胡同'];

function generateRealisticAddress() {
  const province = Random.pick(provinces);
  const city = Random.pick(citiesByProvince[province] || [province.replace('省', '') + '市']);
  let district = '';
  if (districtsByCity[city]) {
    district = Random.pick(districtsByCity[city]);
  } else if (city.endsWith('市') && !['北京市市辖区', '上海市市辖区'].includes(city)) {
     district = Random.pick(['城东区', '城西区', '高新区', '经济开发区', '市中区', `${Random.word(2)}区`]);
  } else if (city.endsWith('市辖区')) { // For Beijing, Shanghai
      district = Random.pick(districtsByCity[province + city] || districtsByCity[city]);
  }


  const street = Random.pick(streetNames);
  const number = Random.integer(1, 800);
  const roadType = Random.pick(roadTypes);
  const buildingOrCommunity = Random.pick(['', ` ${Random.word(2,4)}${Random.pick(streetSuffixes)}`, ` ${Random.word(2,4)}小区`]);
  const room = Random.pick(['', ` ${Random.integer(1,30)}栋${Random.integer(101,2508)}室`, ` ${Random.integer(1,50)}号楼${Random.integer(1,20)}单元${Random.integer(101,808)}`]);

  return `${province}${city}${district}${street}${roadType}${number}号${buildingOrCommunity}${room}`.replace(/undefined/g, '').trim();
}
...
const cargoDetailsOptions = ['成品电脑 (型号: @word(5)-@natural(1000,2000))', '品牌服装 (批次: @id)', '组合家具 (订单号: @natural(1,100))', '进口水果 (原产地: @country)', '畅销书籍 (系列: @word(3))', '高端护肤品套装', '婴儿配方奶粉 (阶段: @natural(1,3))', '户外帐篷 (适用人数: @natural(2,6))'];
const shelfPrefixes = ['A', 'B', 'C', 'D', 'E'];
const customerNames = ['王芳', '李勇', '刘杰', '陈秀英', '张明', '赵丽', '孙悦', '周涛', '钱进', '吴敏'];
const commonOperators = ['拣货员_A组', '拣货员_B组', '复核员_01', '系统自动出库', '仓库主管'];

for (let i = 0; i < count; i++) {
  const id = count - i;
  const reverseIndexForTime = count - 1 - i;
  const createTimestamp = generatePrimaryTimestamp(reverseIndexForTime, count);
  const outboundTimestamp = generateFutureTimestamp(createTimestamp, 0, 1); // Outbound within 0-1 day of creation
  const updateTimestamp = generateFutureTimestamp(outboundTimestamp, 0, 2); // Update within 0-2 days of outbound

  list.push(Mock.mock({
    id: id,
    orderNumber: `OUT${Random.date('yyyyMMdd')}${Random.string('number', 5)}`,
    ...
    shelfNumber: `${Random.pick(shelfPrefixes)}-${Random.integer(1, 6)}-${Random.string('number', 2).padStart(2,'0')}`,
...
module.exports = [
  ...
  },
  {
    ...
    response: config => {
      ...
      const newItem = {
        id: maxId + 1,
        orderNumber: data.orderNumber || `OUT${Random.date('yyyyMMdd')}${Random.string('number', 5)}`,
        ...
        shelfNumber: data.shelfNumber || `${Random.pick(shelfPrefixes)}-${Random.integer(1, 6)}-${Random.string('number', 2).padStart(2,'0')}`,
        createTime: formatTimestamp(createTimestamp),
        outboundTime: formatTimestamp(outboundTimestamp),
        updateTime: formatTimestamp(updateTimestamp),
